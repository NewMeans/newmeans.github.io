/* NewMeans — lightweight i18n (KO/EN)
 * Text lives here; markup carries data-i18n="key".
 * For attributes, add data-i18n-attr="content|aria-label" alongside data-i18n.
 * Other scripts read strings via window.i18n.t(key) and listen for the
 * "nm:langchange" event to refresh anything set dynamically. */
(function () {
  "use strict";

  var STRINGS = {
    ko: {
      "meta.title": "NewMeans — 게임과 AI를 만드는 2인 스튜디오",
      "meta.description": "NewMeans는 게임과 AI를 둘이서 만들고 운영하는 작은 스튜디오입니다.",
      "nav.skip": "본문으로 건너뛰기",
      "nav.works": "제품",
      "nav.studio": "소개",
      "nav.backHome": "← NewMeans",
      "home.h1a": "게임과 AI를",
      "home.h1b": "둘이서 만들어요",
      "home.sub": "만들고 싶은 걸 직접 만들어 세상에 내놓고, 사람들 반응을 보며 고쳐 나가요. 지금은 타건음 게임 Typer와 AI 심리테스트 도파민대학교, 두 개를 운영하고 있어요.",
      "home.cta1": "제품 보러가기",
      "home.cta2": "메일 보내기",
      "home.works.title": "요즘 만드는 것",
      "home.works.note": "둘이서 만들어 직접 운영하는 제품 두 개예요.",
      "home.typer.kind": "모바일 게임 · iOS / Android",
      "home.typer.desc": "기계식 키보드 소리로 노는 게임이에요. 숫자 블록을 공으로 부수고, 스위치마다 다른 타건음을 듣고, 코인을 모아 내 책상을 꾸며요.",
      "home.typer.more": "Typer 보기",
      "home.dopa.kind": "AI 심리테스트 · Web",
      "home.dopa.desc": "카카오톡 대화를 넣으면 AI가 성향을 분석해서, 나만의 웃긴 심리 리포트를 써주는 웹 심리테스트예요.",
      "home.dopa.more": "도파민대학교 보기",
      "home.dopa.who": "카카오톡 대화",
      "home.dopa.tag": "AI 분석 중",
      "home.dopa.m1": "나 요즘 자꾸 밤에 폰만 봐 ㅠ",
      "home.dopa.m2": "오 그거 도파민 신호일지도?",
      "home.dopa.result": "분석 완료 · 자극 추구형",
      "home.dopa.resultSub": "AI 심리 리포트 생성됨",
      "home.studio.title": "우리 둘",
      "home.studio.lead1": "큰 팀이 할 일을",
      "home.studio.lead2": "작은 팀이 끝까지",
      "home.studio.lead3": "해내는 걸 좋아해요.",
      "home.studio.p1": "게임 클라이언트 개발자와 AI 엔지니어, 둘이서 기획부터 개발·디자인·운영까지 나눠 맡아요. 만들고 싶은 게 생기면 바로 만들어보고, 별로면 빠르게 접고, 괜찮으면 오래 다듬어요.",
      "home.studio.p2": "게임이든 AI든 결은 달라도 목표는 같아요. 사람들이 즐겁게 쓰는 것.",
      "home.role1.n": "클라이언트 · 게임",
      "home.role1.d": "기획 · 게임 개발 · 디자인 · 운영",
      "home.role2.n": "AI · 서버",
      "home.role2.d": "AI · 풀스택 · 빌드/배포 · QA 자동화",
      "foot.say": "게임과 AI를 만드는 2인 스튜디오. 만들고 싶은 걸 만들어요.",
      "foot.products": "제품",
      "foot.dopa": "도파민대학교",
      "foot.contact": "연락",
      "foot.madeby": "둘이서 만들었어요",
      "sound.enable": "소리 켜기",
      "sound.disable": "소리 끄기",
      "typer.meta.title": "Typer — NewMeans",
      "typer.meta.description": "Typer는 기계식 키보드 소리를 중심에 둔 감각형 캐주얼 게임입니다. NewMeans 제작.",
      "typer.eyebrow": "게임 · iOS / Android",
      "typer.tagline": "부수고, 듣고, 꾸미는 재미.",
      "typer.lead": "기계식 키보드 소리를 중심에 둔 감각형 캐주얼 게임. 화면 속 키보드를 눌러보세요.",
      "typer.hint": "키보드를 눌러보세요. 오른쪽 위에서 소리를 켤 수 있어요.",
      "typer.play.kicker": "직접 해보기",
      "typer.play.title": "브라우저에서 바로 플레이",
      "typer.play.lead": "숫자 블록을 공으로 부수는 Typer의 코어 루프. 드래그해서 조준하고, 손을 놓으면 공이 발사됩니다.",
      "typer.play.r1": "드래그해서 각도를 잡습니다.",
      "typer.play.r2": "손을 놓으면 공이 우르르 발사됩니다.",
      "typer.play.r3": "블록이 바닥에 닿기 전에 모두 부수세요.",
      "typer.metricsCap": "유료 마케팅 없이",
      "typer.m1": "오가닉 다운로드",
      "typer.m2": "평균 평점",
      "typer.m3": "출시 국가",
      "typer.m4": "리뷰 언어권",
      "typer.pillars.title": "부수고, 듣고, 꾸미고",
      "typer.p1.k": "부수고",
      "typer.p1.d": "숫자가 적힌 키캡 블록을 공으로 부숩니다. 짧고 부드러운 반복.",
      "typer.p2.k": "듣고",
      "typer.p2.d": "스위치마다 다른 타건음. 스위치는 외형이 아니라 ‘청각적 스킨’입니다.",
      "typer.p3.k": "꾸미고",
      "typer.p3.d": "키캡·프레임·모니터를 모아 나만의 책상을 만듭니다. 10만 가지가 넘는 조합.",
      "typer.gallery.title": "게임 속 풍경",
      "typer.foot.title": "설치하고, 스위치를 고르고, 나머지는 책상에 맡기세요.",
      "dopa.meta.title": "도파민대학교 — NewMeans",
      "dopa.meta.description": "도파민대학교는 대화를 분석해 연구 보고서를 써주는 AI 심리테스트 플랫폼입니다. NewMeans 제작.",
      "dopa.eyebrow": "AI 심리테스트 · Web",
      "dopa.hero.t1": "대화를 넣으면,",
      "dopa.hero.t2": "AI가 당신을 연구합니다.",
      "dopa.lead": "카카오톡 대화만 올리면 AI가 성향을 분석해 나만의 심리 리포트를 써주는 초개인화 심리테스트 플랫폼.",
      "dopa.cta": "dodae.me에서 시작하기",
      "dopa.chat.who": "카카오톡 대화",
      "dopa.chat.tag": "AI 분석 중",
      "dopa.chat.m1": "나 사실 요즘 좀 지쳤어",
      "dopa.chat.m2": "헐 무슨 일 있어?? 😥",
      "dopa.chat.m3": "아니 그냥… 다 귀찮아 ㅎㅎ",
      "dopa.report.title": "분석 완료 · 에겐 성향",
      "dopa.report.sub": "AI 심리 리포트 생성됨",
      "dopa.report.left": "에겐 74%",
      "dopa.report.right": "테토 26%",
      "dopa.what.title": "무엇을 하나요",
      "dopa.prof.teto.label": "테토 · TETO",
      "dopa.prof.teto.name": "테토 교수",
      "dopa.prof.teto.desc": "직진 · 주도 · 에너지",
      "dopa.prof.egen.label": "에겐 · EGEN",
      "dopa.prof.egen.name": "에겐 교수",
      "dopa.prof.egen.desc": "섬세 · 공감 · 사색",
      "dopa.what.note": "대표 콘텐츠는 에겐-테토 테스트. 카카오톡 대화를 올리면 AI가 참여자 각자의 성향을 분석해 웃긴 연구 보고서를 만들어 줍니다. MBTI처럼 정해진 유형이 아니라, 참여한 사람 수만큼 다른 결과가 나옵니다 — 2,000명이 하면 2,000개의 결과.",
      "dopa.test.kicker": "미리 해보기",
      "dopa.test.title": "나는 에겐? 테토?",
      "dopa.test.sub": "3문항 미리보기. 정식 테스트에선 AI가 당신의 대화를 분석합니다.",
      "dopa.test.retry": "다시 해보기",
      "dopa.test.full": "정식 테스트 하러 가기",
      "dopa.test.resultTeaser": "미리보기 결과예요. 정식 리포트는 AI가 당신의 대화를 읽고 훨씬 자세히 써줍니다.",
      "dopa.platform.t1": "하나의 테스트가 아니라,",
      "dopa.platform.t2": "계속 늘어나는 플랫폼.",
      "dopa.platform.p": "에겐-테토를 시작으로, 좋아하는 만화 캐릭터가 되어보는 테스트처럼 다양한 테마의 캐릭터 심리테스트가 계속 추가됩니다. 전부 같은 AI 분석 엔진 위에서 돌아가요.",
      "dopa.foot.title": "당신에 대한 연구를 시작해 보세요.",
      "dopa.foot.sub": "대화만 넣으면, 나머지는 AI가 합니다."
    },
    en: {
      "meta.title": "NewMeans — a two-person games & AI studio",
      "meta.description": "NewMeans is a small studio where two of us make and run our own games and AI.",
      "nav.skip": "Skip to content",
      "nav.works": "Work",
      "nav.studio": "About",
      "nav.backHome": "← NewMeans",
      "home.h1a": "Two of us,",
      "home.h1b": "making games & AI",
      "home.sub": "We build what we want to build, put it out into the world, and keep fixing it as people use it. Right now we run Typer, a keyboard-sound game, and Dopamine University, an AI psych test.",
      "home.cta1": "See our work",
      "home.cta2": "Say hi",
      "home.works.title": "What we're making",
      "home.works.note": "Two products the two of us build and run ourselves.",
      "home.typer.kind": "Mobile game · iOS / Android",
      "home.typer.desc": "A game you play with mechanical-keyboard sound. Break numbered blocks, hear a different switch on every hit, and deck out your own desk.",
      "home.typer.more": "See Typer",
      "home.dopa.kind": "AI psych test · Web",
      "home.dopa.desc": "Paste a KakaoTalk chat and AI reads the vibe, then writes a psych report that's all yours.",
      "home.dopa.more": "See Dopamine University",
      "home.dopa.who": "KakaoTalk chat",
      "home.dopa.tag": "AI analyzing",
      "home.dopa.m1": "i keep scrolling my phone all night ㅠ",
      "home.dopa.m2": "ooh that might be a dopamine signal?",
      "home.dopa.result": "Done · Novelty-seeker",
      "home.dopa.resultSub": "AI report generated",
      "home.studio.title": "The two of us",
      "home.studio.lead1": "We like taking",
      "home.studio.lead2": "small things all the way",
      "home.studio.lead3": "— the work a big team would.",
      "home.studio.p1": "A game-client developer and an AI engineer. Between us we split the planning, the code, the design, and the running of it. If we want to make something we just make it, drop it fast if it's meh, and polish it for a long time if it's good.",
      "home.studio.p2": "Game or AI, different on the surface, same goal underneath: something people enjoy using.",
      "home.role1.n": "Client · Games",
      "home.role1.d": "Product · game dev · design · ops",
      "home.role2.n": "AI · Backend",
      "home.role2.d": "AI · full-stack · build/deploy · QA automation",
      "foot.say": "A two-person studio making games & AI. We make what we want to make.",
      "foot.products": "Work",
      "foot.dopa": "Dopamine University",
      "foot.contact": "Contact",
      "foot.madeby": "Made by the two of us",
      "sound.enable": "Turn sound on",
      "sound.disable": "Turn sound off",
      "typer.meta.title": "Typer — NewMeans",
      "typer.meta.description": "Typer is a tactile casual game built around mechanical-keyboard sound. Made by NewMeans.",
      "typer.eyebrow": "Game · iOS / Android",
      "typer.tagline": "Break. Listen. Customize.",
      "typer.lead": "A tactile casual game built around mechanical-keyboard sound. Tap the keyboard on screen.",
      "typer.hint": "Tap the keyboard. Turn sound on at the top right.",
      "typer.play.kicker": "Try it",
      "typer.play.title": "Play it right in your browser",
      "typer.play.lead": "Typer's core loop: break the numbered blocks with a bounce of balls. Drag to aim, let go to fire.",
      "typer.play.r1": "Drag to set your angle.",
      "typer.play.r2": "Let go and the balls launch in a burst.",
      "typer.play.r3": "Clear every block before one reaches the floor.",
      "typer.metricsCap": "with zero paid marketing",
      "typer.m1": "organic downloads",
      "typer.m2": "average rating",
      "typer.m3": "countries",
      "typer.m4": "review languages",
      "typer.pillars.title": "Break, listen, customize",
      "typer.p1.k": "Break",
      "typer.p1.d": "Smash numbered keycap blocks with a bounce of balls. Short, smooth loops.",
      "typer.p2.k": "Listen",
      "typer.p2.d": "Every switch sounds different. A switch isn't a look, it's an audio skin.",
      "typer.p3.k": "Customize",
      "typer.p3.d": "Collect keycaps, frames, and monitors to build your own desk. 100,000+ combinations.",
      "typer.gallery.title": "Inside the game",
      "typer.foot.title": "Install it, pick a switch, and let the desk do the rest.",
      "dopa.meta.title": "Dopamine University — NewMeans",
      "dopa.meta.description": "Dopamine University is an AI personality-test platform that turns a chat into a research report. Made by NewMeans.",
      "dopa.eyebrow": "AI psych test · Web",
      "dopa.hero.t1": "Drop in a chat,",
      "dopa.hero.t2": "and AI studies you.",
      "dopa.lead": "A hyper-personalized psych-test platform: upload a chat and AI reads the vibe, then writes a report that's all yours.",
      "dopa.cta": "Start at dodae.me",
      "dopa.chat.who": "KakaoTalk chat",
      "dopa.chat.tag": "AI analyzing",
      "dopa.chat.m1": "honestly i've been drained lately",
      "dopa.chat.m2": "omg what happened?? 😥",
      "dopa.chat.m3": "nah just… over everything lol",
      "dopa.report.title": "Done · Egen-leaning",
      "dopa.report.sub": "AI psych report generated",
      "dopa.report.left": "Egen 74%",
      "dopa.report.right": "Teto 26%",
      "dopa.what.title": "What it does",
      "dopa.prof.teto.label": "Teto · TETO",
      "dopa.prof.teto.name": "Prof. Teto",
      "dopa.prof.teto.desc": "Driven · bold · high-energy",
      "dopa.prof.egen.label": "Egen · EGEN",
      "dopa.prof.egen.name": "Prof. Egen",
      "dopa.prof.egen.desc": "Sensitive · warm · reflective",
      "dopa.what.note": "The flagship is the Egen–Teto test. Upload a KakaoTalk chat and AI analyzes each person and writes a comedic research report. No fixed types like MBTI — you get as many results as there are people: 2,000 players, 2,000 results.",
      "dopa.test.kicker": "Try a mini test",
      "dopa.test.title": "Egen or Teto?",
      "dopa.test.sub": "A 3-question preview. In the full test, AI analyzes your actual chats.",
      "dopa.test.retry": "Try again",
      "dopa.test.full": "Take the full test",
      "dopa.test.resultTeaser": "This is just a preview. The full report has AI read your chats and go much deeper.",
      "dopa.platform.t1": "Not one test —",
      "dopa.platform.t2": "a platform that keeps growing.",
      "dopa.platform.p": "Starting with Egen–Teto, new themed character tests keep landing — like becoming your favorite comic character. All of them run on the same AI analysis engine.",
      "dopa.foot.title": "Start the research into you.",
      "dopa.foot.sub": "Just drop in a chat — AI does the rest."
    }
  };

  var STORAGE_KEY = "nm-lang";
  var SUPPORTED = ["ko", "en"];

  function detectLang() {
    try {
      var saved = window.localStorage.getItem(STORAGE_KEY);
      if (saved && SUPPORTED.indexOf(saved) !== -1) return saved;
    } catch (e) { /* private mode */ }
    var nav = (navigator.language || "en").toLowerCase();
    return nav.indexOf("ko") === 0 ? "ko" : "en";
  }

  var current = detectLang();

  function t(key) {
    var table = STRINGS[current] || STRINGS.en;
    return table[key] != null ? table[key] : (STRINGS.en[key] != null ? STRINGS.en[key] : key);
  }

  function apply(lang) {
    if (SUPPORTED.indexOf(lang) === -1) lang = "en";
    current = lang;

    try { window.localStorage.setItem(STORAGE_KEY, lang); } catch (e) { /* ignore */ }
    document.documentElement.setAttribute("lang", lang);

    var nodes = document.querySelectorAll("[data-i18n]");
    for (var i = 0; i < nodes.length; i++) {
      var el = nodes[i];
      var value = t(el.getAttribute("data-i18n"));
      var attr = el.getAttribute("data-i18n-attr");
      if (attr) {
        el.setAttribute(attr, value);
      } else {
        el.textContent = value;
      }
    }

    // Reflect active state on the language toggle.
    var buttons = document.querySelectorAll("[data-lang-set]");
    for (var j = 0; j < buttons.length; j++) {
      var btn = buttons[j];
      var isActive = btn.getAttribute("data-lang-set") === lang;
      btn.classList.toggle("is-active", isActive);
      btn.setAttribute("aria-pressed", isActive ? "true" : "false");
    }

    // Let dynamic scripts (sound label, etc.) refresh.
    window.dispatchEvent(new CustomEvent("nm:langchange", { detail: { lang: lang } }));
  }

  function wireToggle() {
    var buttons = document.querySelectorAll("[data-lang-set]");
    for (var i = 0; i < buttons.length; i++) {
      buttons[i].addEventListener("click", function () {
        apply(this.getAttribute("data-lang-set"));
      });
    }
  }

  window.i18n = { t: t, lang: function () { return current; }, set: apply };

  function init() {
    wireToggle();
    apply(current);
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", init);
  } else {
    init();
  }
})();
