...
(function () {
  ...
  var cur = {}, cat = null, typed = '', lines = {}, timers = {};
  function t(k) { return window.i18n ? window.i18n.t(k) : k; }

  // ------------------------------------------------ monitor text
  var ORDER = ['keycap', 'frame', 'monitor', 'switch', 'sculpture', 'ball'], LABEL = { keycap: 'keycap ', frame: 'frame  ', monitor: 'monitor', switch: 'switch ', sculpture: 'object ', ball: 'ball   ' };
  function render() {
    var out = ['TYPER'];
    ORDER.forEach(function (k) { out.push('> ' + LABEL[k] + ' ' + (lines[k] || '')); });
    text.textContent = out.join('\n') + '\n' + typed;
    var c = document.createElement('span'); c.className = 'caret'; text.appendChild(c);
  }
  function typeLine(k, name) {
    clearTimeout(timers[k]);
    if (reduce) { lines[k] = name; render(); return; }
    var i = 0; lines[k] = '';
    (function step() { lines[k] = name.slice(0, ++i); render(); if (i < name.length) timers[k] = setTimeout(step, 28); })();
  }
  ...
  function apply(kind, item, silent) {
    ...
    if (!silent) { typeLine(kind, item.name); TyperKeyboard.play(kb); } else { lines[kind] = item.name; render(); }
  }
  ...
  function build(kind, items) {
    ...
    items.forEach(function (it) {
      var b = document.createElement('button'); b.type = 'button'; b.className = 'chip' + (kind === 'switch' ? ' chip--swatch' : ''); b.dataset.id = it.id; b.title = it.name; b.setAttribute('aria-label', it.name); b.setAttribute('aria-pressed', 'false');
      var im = new Image(); im.src = SHOP + it.thumb; im.alt = ''; im.loading = 'lazy'; b.appendChild(im);
  ...
  kb.addEventListener('tp:key', function (e) {
    var ch = KEYS[e.detail.index] || ''; typed += ch; if (typed.length > 22) typed = ''; render();
  });
